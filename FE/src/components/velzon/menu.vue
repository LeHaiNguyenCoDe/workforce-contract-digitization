...
const initActiveMenu = (path?: string) => {
  ...
  if (ul) {
    ...
    if (matchingMenuItem) {
      ...
      let parent = matchingMenuItem.closest(".collapse") as HTMLElement;
      while (parent) {
        ...
        parent = parent.parentElement?.closest(".collapse") as HTMLElement;
      }
    }
  }
};
...
onMounted(() => {
  ...
  // Initialize collapse menu functionality
    ...
    link.addEventListener('click', (e) => {
      ...
      if (target) {
        ...
        if (collapseElement) {
          ...
          if (parent) {
            const siblings = parent.parentElement?.querySelectorAll(':scope > .nav-item > .menu-link[aria-expanded="true"], :scope > .nav-item > .nav-link[aria-expanded="true"]');
            siblings?.forEach((sibling) => {
              if (sibling !== link) {
                sibling.setAttribute('aria-expanded', 'false');
                const siblingTarget = sibling.getAttribute('href');
                if (siblingTarget) {
                  document.querySelector(siblingTarget)?.classList.remove('show');
                }
              }
            });
          }
...
<template>
  <BContainer fluid>
    ...
    <template v-if="layoutType === 'vertical' || layoutType === 'semibox' || layoutType === 'horizontal' || layoutType === 'twocolumn'">
