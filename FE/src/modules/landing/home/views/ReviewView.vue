    ...
    <section class="py-16 overflow-hidden">
        <div>
            <div class="text-center mb-12">
                <h2 class="text-4xl font-bold text-[#9F7A5F] mb-3">{{ t('home.reviewTitle') || 'Đánh giá từ người dùng' }}</h2>
                <p class="text-gray-600 text-lg">{{ t('home.reviewDesc') || 'Những sản phẩm được đánh giá tốt từ người dùng' }}</p>
            </div>
            <div v-if="reviewStore.isLoading" class="text-center py-16 text-gray-500">{{ t('common.loading') || 'Đang tải...' }}</div>
            ...
            <div v-else-if="reviewStore.hasFeaturedReviews" class="relative flex bg-[#C4C4C480] py-12">
                    ...
                    <swiper-slide v-for="review in reviewStore.featuredReviews" :key="review.id">
                        <div class="bg-white rounded-sm shadow-md h-full flex flex-col overflow-hidden">
                            <div class="h-60 overflow-hidden">
                                <img :src="review.image" :alt="review.author" class="w-full h-full object-cover hover:scale-105 transition-transform duration-300">
                            </div>
                            <div class="p-2 flex flex-col flex-grow">
                                <div class="flex gap-1">
                                    <img v-for="index in renderStarsArray(review.rating)" :key="index" src="../../../../assets/landing/home/star.svg" alt="star" class="w-4 h-4">
                                </div>
                ...
                <!-- Custom navigation -->
                <button class="review-prev absolute right-16 top-1 z-10 w-10 h-10 rounded-full bg-white border-2 border-gray-300 shadow-lg flex items-center justify-center hover:border-[#9F7A5F] hover:text-[#9F7A5F] transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-[#9F7A5F]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
						<path d="m15 18-6-6 6-6" />
					</svg>
                </button>
                <button class="review-next absolute right-4 top-1 z-10 w-10 h-10 rounded-full bg-white border-2 border-gray-300 shadow-lg flex items-center justify-center hover:border-[#9F7A5F] hover:text-[#9F7A5F] transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-[#9F7A5F]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
						<path d="m9 6 6 6-6 6" />
					</svg>
                </button>
