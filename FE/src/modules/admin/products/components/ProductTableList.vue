  ...
  <BCard no-body class="border-0 shadow-sm mb-4 overflow-hidden" style="border-radius: 8px;">
    <BCardBody class="p-3 pb-0">
      <BRow class="g-3 align-items-center mb-3">
        <BCol>
          <BButton variant="success" @click="handleCreate" class="d-flex align-items-center gap-2 px-3 fw-medium"
            style="background-color: #0ab39c; border-color: #0ab39c;">
      ...
      <div class="d-flex align-items-center justify-content-between border-bottom">
        <BNav tabs class="nav-tabs-custom border-0">
          <BNavItem :active="activeTab === 0" @click="activeTab = 0">
            All <BBadge variant="primary" class="bg-primary-subtle text-primary ms-1 fs-11 px-1 rounded-1">{{
              store.products.length }}</BBadge>
          </BNavItem>
          <BNavItem :active="activeTab === 1" @click="activeTab = 1">
            Published <BBadge variant="danger" class="bg-danger-subtle text-danger ms-1 fs-11 px-1 rounded-1">{{
              store.products.filter(p => p.is_active).length }}</BBadge>
          </BNavItem>
    ...
    <BCardBody>
      <DTable :columns="columns" :data="products" :loading="isLoading" row-key="id" hoverable class="dtable">
        <!-- Selection -->
          ...
          <div class="d-flex align-items-center">
            ...
            <div class="flex-grow-1 overflow-hidden">
              <h5 class="fs-14 mb-1 text-truncate">
                <BLink @click="handleEdit(item.id)" class="text-dark fw-semibold text-decoration-none">{{ item.name }}
                </BLink>
              </h5>
              <p class="text-muted mb-0 small text-truncate">Category : <span class="text-primary">{{
                item.category?.name || 'N/A' }}</span></p>
            </div>
          </div>
        </template>
            ...
            <BRow class="align-items-center g-3 text-center text-sm-start">
              <BCol sm>
                <div class="text-muted small">
                  Showing <span class="fw-semibold">{{ (currentPage - 1) * 10 + 1 }} to {{ Math.min(currentPage * 10,
                    products.length) }}</span> of {{ products.length }} results
                </div>
              </BCol>
              <BCol sm="auto">
                <div class="pagination-wrap">
                  <BPagination v-model="currentPage" :total-rows="products.length" :per-page="10"
                    @update:model-value="(val) => handleChangePage(Number(val))" class="mb-0" size="sm"
