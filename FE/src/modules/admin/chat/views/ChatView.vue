<template>
  <div class="chat-page-container flex flex-col bg-gray-50">
    <div class="flex flex-1 overflow-hidden bg-white shadow-sm border border-gray-100">
      <!-- Sidebar -->
      <aside
        class="absolute md:relative inset-y-0 left-0 z-20 w-full sm:w-80 md:w-72 lg:w-80 flex flex-col bg-white border-r border-gray-100 transform transition-transform duration-300 md:translate-x-0"
      ...
      >
        ...
        <div class="p-3">
          <div class="relative">
            ...
            <input
              v-model="searchQuery"
              type="text"
              :placeholder="t('common.chat.search_placeholder')"
        ...
        <!-- Guest Tab -->
        <ConversationList
          v-if="activeListTab === 'guests'"
          :conversations="filteredGuestConversations"
          :selectedId="currentConversation?.id"
          :loading="isLoadingGuestConversations"
          @select="handleSelectConversation"
          @delete="handleDeleteConversation"
        />
      </aside>
      ...
      <div
        v-if="sidebarOpen"
        @click="sidebarOpen = false"
        class="md:hidden fixed inset-0 z-10 bg-black/20"
      />

      <!-- Main Chat Area -->
      <main
        class="flex-1 flex flex-col overflow-hidden min-w-0 bg-[#f0f4f3]"
        style="
          background-image: url('data:image/svg+xml,%3Csvg width=&quot;60&quot; height=&quot;60&quot; viewBox=&quot;0 0 60 60&quot; xmlns=&quot;http://www.w3.org/2000/svg&quot;%3E%3Cg fill=&quot;none&quot; fill-rule=&quot;evenodd&quot;%3E%3Cg fill=&quot;%2326a69a&quot; fill-opacity=&quot;0.03&quot;%3E%3Ccircle cx=&quot;30&quot; cy=&quot;30&quot; r=&quot;4&quot;/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');
        "
      >
        <!-- Mobile menu button -->
        <button
        ...
        <template v-else>
          <div class="flex-1 flex items-center justify-center p-4">
            <div class="text-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 text-teal-200"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
              ...
              <h3 class="text-xl font-medium text-gray-400 mb-2">
                {{ t('common.chat.select_conversation') }}
              </h3>
      ...
      <ConversationDetails
        v-if="currentConversation && showDetails"
        :conversation="currentConversation"
...
import { useConversationDetails } from '../composables'
...
// Use conversation details for blocking/unblocking actions
const { unblockUser: refinedUnblockUser } = useConversationDetails(currentConversation as any)

async function handleUnblockRefined() {
  try {
...
onMounted(async () => {
  ...
  await Promise.all([chatStore.fetchConversations(), loadUsers(), loadGuestConversations()])
})
