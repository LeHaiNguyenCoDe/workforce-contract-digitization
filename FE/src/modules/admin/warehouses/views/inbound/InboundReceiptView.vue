...
const saveReceipt = async () => {
    ...
    if (validItems.length === 0) {
        await swal.warning('Vui lòng chọn ít nhất một sản phẩm')
        return
    }
    ...
    try {
        ...
        if (editingReceipt.value) {
            ...
            await swal.success('Cập nhật phiếu nhập thành công!')
        } else {
            await store.createInboundReceipt(payload)
            await swal.success('Thêm phiếu nhập thành công!')
        }
        showModal.value = false
        await store.fetchInboundReceipts()
    } catch (error: any) {
        console.error('Error saving receipt:', error)
        await swal.error(error.response?.data?.message || 'Có lỗi xảy ra khi lưu phiếu')
    }
}

const approveReceipt = async (id: number) => {
    const confirmed = await swal.confirm('Xác nhận duyệt phiếu nhập này?')
    if (!confirmed) return
    try {
        await store.approveInboundReceipt(id)
        await swal.success('Duyệt phiếu nhập thành công!')
        await store.fetchInboundReceipts()
    } catch (error: any) {
        console.error('Approve error:', error)
        await swal.error(error.response?.data?.message || 'Có lỗi xảy ra')
    }
}

const cancelReceipt = async (id: number) => {
    const confirmed = await swal.confirm('Xác nhận hủy phiếu nhập này?')
    if (!confirmed) return
    try {
        await store.cancelInboundReceipt(id)
        await swal.success('Hủy phiếu nhập thành công!')
        await store.fetchInboundReceipts()
    } catch (error: any) {
...
const deleteReceipt = async (id: number) => {
    const confirmed = await swal.confirmDelete('Xác nhận xóa phiếu nhập này? Thao tác này không thể hoàn tác.')
    if (!confirmed) return
    try {
        await store.deleteInboundReceipt(id)
        await swal.success('Xóa phiếu nhập thành công!')
        await store.fetchInboundReceipts()
    } catch (error: any) {
        console.error('Delete error:', error)
        await swal.error(error.response?.data?.message || 'Có lỗi xảy ra khi xóa')
    }
}
...
const createBatchFromReceipt = async (receipt: any) => {
    ...
    router.push({ name: 'admin-warehouse-inbound-batches' })
}
