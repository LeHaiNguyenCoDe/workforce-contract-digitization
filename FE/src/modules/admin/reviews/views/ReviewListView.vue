  ...
  <div class="h-full flex flex-col p-6">
    ...
    <div class="bg-dark-800 rounded-xl border border-white/10 p-4 mb-6 flex-shrink-0">
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <button class="btn btn-secondary" @click="store.fetchReviews({ status: statusFilter || undefined })">
    ...
    <div class="flex-1 min-h-0 bg-dark-800 rounded-xl border border-white/10 overflow-hidden flex flex-col">
      ...
      <div v-else class="flex-1 overflow-auto divide-y divide-white/5">
        <div v-for="review in filteredReviews" :key="review.id" class="p-6 hover:bg-white/5 transition-colors">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-4 mb-2">
                <div>
                  <p class="font-medium text-white">
                    ...
                    <span v-if="review.is_admin_reply"
                      class="ml-2 px-1.5 py-0.5 bg-primary/20 text-primary-light text-[10px] rounded uppercase">
                      Phản hồi
                    </span>
                  </p>
                ...
                <div class="flex items-center gap-2">
                  <span :class="['inline-flex items-center px-3 py-1 rounded-full text-xs font-medium', getStatusColor(review.status)]">
              ...
              <div class="flex items-center gap-1 mb-2">
                <svg v-for="i in 5" :key="i" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                  ...
                  :class="i <= review.rating ? 'text-yellow-400' : 'text-slate-600'" stroke="currentColor"
                  stroke-width="2">
                  <polygon
              ...
              <div class="mt-4" v-else>
                <button @click="deleteReview(review.id)" class="text-sm text-error hover:underline">
                  Xóa đánh giá
                </button>
              </div>
            </div>
        ...
        <div v-if="!filteredReviews.length" class="py-16 text-center">
